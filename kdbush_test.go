...
var testGeoPoints = [][]float64{
	{38.902532, -77.011624}, {38.902419, -77.010401}, {38.902348, -77.011839}, {38.902465, -77.020143}, {38.899753, -77.022160},
	{38.902493, -77.007098}, {38.904327, -77.009855}, {38.906837, -77.013943}, {38.908124, -77.021582}, {38.907014, -77.029424},
}
...
func getTestGeoPoints() []Point {
	var points = make([]Point, len(testGeoPoints))
	for i := range testGeoPoints {
		points[i] = &SimplePoint{X: testGeoPoints[i][0], Y: testGeoPoints[i][1]}
	}
	return points
}
...
func ExampleKDBush_Within() {
	...
	bush := NewBush(points, 10)
	result := bush.Within(&SimplePoint{15, 15}, 10, false)
	fmt.Println(result)
	// Output: [0 1 3 5]
}
...
func TestKDBush_Within(t *testing.T) {
	...
	result := bush.Within(&point, 20, false)
...
func TestKdBushWithinHaversine(t *testing.T) {
	var points = getTestGeoPoints()
	bush := NewBush(points, 10)

	point := SimplePoint{X: 38.902511, Y: -77.009382}
	result := bush.Within(&point, 200, true)
	expectedResult := []int{0, 1, 5}
	assert.Equal(t, expectedResult, result)
}
